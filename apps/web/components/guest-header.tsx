...
export default function GuestHeader() {
  ...
  return (
    <header className="relative z-30 flex h-14 shrink-0 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur sm:px-6">
      <Logo />
        ...
        {showCta && (
          <div className="relative">
            <Link
              href="/whatsapp"
              className={cn(
                "flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#1ebe5c]",
            ...
            {showCoachmark && (
              <div className="absolute right-0 top-[calc(100%+10px)] w-[min(90vw,20rem)] rounded-2xl bg-[#2563eb] p-4 text-white shadow-2xl">
                {/* стрелка-указатель к кнопке */}
                <div className="absolute -top-1.5 right-8 h-3 w-3 rotate-45 bg-[#2563eb]" />
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="-mr-1 -mt-1 rounded-full p-1 text-white/70 transition hover:bg-white/10 hover:text-white"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
