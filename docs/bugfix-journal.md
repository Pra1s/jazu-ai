...
## 2026-07-18 — Ревью фичи «бот в стиле владельца»: 8 багов (фича не работала в проде)

- **Что случилось:** ревью только что реализованной фичи анализа WhatsApp-диалогов выявило, что оба продуктовых пути (загрузка файлов и «из WhatsApp») всегда падали в `error` «мало карточек», плюс ряд проблем с потерей ввода, порчей данных маской и перезапуском.
- **Почему:**
  - **C1 (critical):** эпизоды сохранялись в `StyleAnalysis.episodes` (JSONB) → `turns[].timestamp: Date` сериализовался в ISO-строку; хендлер читал её кастом без ревайва, а `formatEpisodeForPrompt` звал `.getTime()` на строке → `TypeError` на КАЖДОМ эпизоде с таймингами (т.е. почти всегда) → 0 карточек. CLI не ловил, т.к. держал эпизоды в памяти с настоящими Date.
  - **M1:** `maskPhones` матчил ≥7 цифр с разделителями → съедал цены «5 000 000 тенге» и перечисления «42 44 46 48».
  - **M2:** `add` без `removeOnFail` → упавшая задача оставалась в failed-set (до 7 дней), повторный `add` с тем же `jobId style:<agentId>` — no-op → перезапуск невозможен.
  - **M3:** буфер `WaHistoryChat` удалялся сразу после enqueue, а при ошибке прогона `episodes` затирались → history-ввод терялся безвозвратно (переприслать нельзя).
  - **M4:** `isOwnerSender` сравнивал имена двусторонней подстрокой → «Али» матчил «Галия», реплики клиента метились владельцем.
  - **M5:** internal-роут `upsert`-ил `messages` перезаписью → чанки history-sync затирали друг друга (WhatsApp шлёт историю несколькими непересекающимися событиями).
  - **M6:** повторная загрузка во время идущего прогона перезаписывала буфер, но `add` был no-op → новый ввод терялся, старая задача выставляла «done».
  - **M7:** `pushHistoryDialogues` не проверял `response.ok` и не резал сообщения по 5000 → чанк с >5000 сообщений отвергался `ZodError` молча.
- **Как решили:** `reviveEpisodeDates` (`packages/ai/src/style/parse.ts`) оживляет Date после JSONB, вызывается в `runStyleAnalysis`; `maskPhones` переписан с эвристикой телефона (+/скобки/дефис, 10+ слитных цифр, отсев групп по 3); `enqueueStyleAnalysis` c `removeOnFail: true`; `clearHistoryOnSuccess` в `StyleAnalyzeJob` — буфер истории чистит хендлер только при успехе; `isOwnerSender` сравнивает по целым словам-токенам; internal-роут мерджит сообщения (append+dedupe по ts+fromMe+text, кап 5000); роуты загрузки/сброса отдают 409 при идущем анализе (`isStyleAnalysisRunning`); воркер режет диалог по 5000 и логирует не-ok ответ. Плюс minor: персона/длина в `buildRuntimeEnvelope` переопределяются только при `styleGuide` (согласовано со status/RAG-гейтами), `embedTexts` по умолчанию идёт на chat-endpoint, `SYSTEM_NOTICE_RE` сужен («сменил номер телефона»), сортировка сообщений без ts не улетает в начало (carry-forward), CLI-агент выбирается детерминированно.
- **Как проверить / не повторить:** `pnpm --filter @jazu/ai exec vitest run` — новые тесты на цены, определение владельца (Али/Галия), round-trip Date, «сменил номер»; typecheck/lint зелёные по всему репо. При изменении хранения эпизодов помнить: Date после JSON round-trip = строка, всегда ревайвить перед арифметикой.

## 2026-07-14 — 500 при вставке большого скрипта продаж в «Данные о бизнесе»

- **Что случилось:** при сохранении диалога «Данные о бизнесе» с большим текстом в поле «Скрипт / сценарий продаж» фронт показывал «Request failed: 500».
- **Почему:** в `POST /agent/extra-data` поле `script` было ограничено 4000 символов в zod-схеме (`apps/api/src/routes.ts`), а `extraDataSchema.parse()` при превышении кидал `ZodError`, который глобальный `setErrorHandler` (`apps/api/src/server.ts`) отдавал как 500 без внятного сообщения. Аналогично маскировались под 500 и другие клиентские ошибки Fastify (413 body too large, 429 rate-limit).
- **Как решили:** (1) лимит `script` поднят до 20 000 символов с русским сообщением об ошибке. (2) `setErrorHandler` теперь маппит `ZodError` → 400 с первым сообщением issue, а ошибки с `statusCode` 4xx отдаёт их статусом (413/429 и т.п.) без отправки в Sentry — в Sentry уходят только настоящие 5xx. (3) `apiJson` (`apps/web/lib/api.ts`) показывает `message` из тела ответа вместо «Request failed: N». (4) в `extra-data-dialog.tsx` на textarea добавлены `maxLength`, совпадающие с бэкенд-лимитами, и счётчик символов при приближении к лимиту.
- **Как проверить / не повторить:** вставить текст >4000 символов в скрипт продаж → сохраняется (до 20 000, дальше textarea не даёт ввести). Невалидное тело на любом zod-роуте → 400 с сообщением (проверено inject-тестом: `POST /auth/magic-link` c `email: 123` → 400, тело >256 KB → 413). При добавлении новых полей в `extraDataSchema` держать `maxLength` на фронте равным лимиту схемы.
