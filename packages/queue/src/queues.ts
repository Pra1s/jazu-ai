...
export type WaOutboundJob = {
  ...
  sentCount?: number;
  /**
   * Сколько раз задача уже переставлялась заново после выгорания всех попыток
   * с недоставленным хвостом. Ограничивает «вечный» цикл добивки: пока сокет не
   * вернулся дольше, чем живут ретраи (~5 мин), остаток переставляется с растущей
   * паузой, но не более WA_OUTBOUND_REQUEUE_MAX раз.
   */
  requeueCount?: number;
